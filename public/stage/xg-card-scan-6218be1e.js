/* Card scanner (beta) — point the camera at a card, we read the title line
   ...
   the worker's code mode). Two gates keep that honest: nothing renders
   without a LIVE STOCK match, and a set+number identification must be
   CORROBORATED by the artist credit in the same photo — a misread number
   lands on a real, in-stock, WRONG card otherwise (run 87). The read line
   names the collector-line basis whenever it was the finder. */
(function () {
  ...
  /* A misread collector NUMBER resolves to a REAL card: run 87 read C21 #9
     off a blacked-title Sol Ring photo, Scryfall returned Adrix and Nev,
     the shelf had it, and the scanner rendered a confidently wrong card
     under an authoritative badge (the same trap as run 83's Dire Mimic,
     and the shelf gate cannot catch it because the wrong card is real and
     stocked). So a set+number identification must CORROBORATE itself: the
     resolved card's ARTIST has to appear in the scanned text. The artist
     credit is printed beside the collector line, so it survives exactly
     the damage this path exists for (unreadable title), while a wrong
     number lands on a card whose artist is nowhere in the photo. Nothing
     to corroborate with -> no render, honest failure instead. */
  function corroborates(sc) {
    var buf = clueBuf.toLowerCase().replace(/[^a-z0-9]+/g, ' ');
    var words = String(sc.artist || '').toLowerCase().split(/[^a-z]+/);
    for (var i = 0; i < words.length; i++) {
      // >=5 letters keeps it distinctive ("bierek" yes, "van"/"de" no)
      if (words[i].length >= 5 && buf.indexOf(words[i]) > -1) return words[i];
    }
    return '';
  }
    ...
    if (game === 'mtg' && clue.set && clue.num) {
      ...
      return fetch('https://api.scryfall.com/cards/' + encodeURIComponent(clue.set) + '/' + encodeURIComponent(String(+clue.num)))
        ...
        .then(function (sc) {
          ...
          var proof = corroborates(sc);
          if (!proof) return null;   // wrong number -> real-but-wrong card
          clue.proof = proof;
