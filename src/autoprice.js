...
/* ---- the same digest by email (owner, 2026-09-22: "can you get 3 working")
   ntfy has been the weak link since 2026-09-15: ntfy.sh throttles by source
   IP, the worker publishes from Cloudflare's shared egress, and the push
   only lands when the GitHub relay picks it up at 22:50 UTC. With
   RESEND_API_KEY on the worker (the same key 9Pocket's customer mail uses)
   the digest is emailed as well, straight from the run - and a delivered
   email settles the digest, so the relay does not push a copy a day later.
   AUTOPRICE_EMAIL_TO is one address or a few, comma-separated; with the key
   set and no list, it goes to the store's own admin address. */
export const DIGEST_EMAIL_DEFAULT = "chaylon@exorgames.com";   // 9Pocket's first-admin address (owner, 2026-09-20)

export function digestEmailTo(env) {
  if (!emailConfigured(env)) return [];
  const raw = String((env && env.AUTOPRICE_EMAIL_TO) || "").trim();
  const list = (raw ? raw.split(/[,;\s]+/) : [DIGEST_EMAIL_DEFAULT]).map((a) => a.trim()).filter((a) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(a));
  return [...new Set(list)].slice(0, 5);
}

export async function sendDigestEmail(cx, d, meta) {
  const to = digestEmailTo(cx.env);
  if (!to.length) return { ok: false, skipped: true, status: "off", error: emailConfigured(cx.env) ? "AUTOPRICE_EMAIL_TO holds no usable address" : "RESEND_API_KEY is not set on the worker" };
  const m = meta || {};
  const stamp = new Date(typeof cx.now === "function" ? cx.now() : Date.now()).toISOString().slice(0, 16).replace("T", " ") + " UTC";
  const note = buildNoticeEmail({
    subject: d.title,
    heading: d.title,
    sub: (m.kind === "test" ? "Test" : m.kind === "manual" ? "Manual run" : "Nightly run") + " · " + stamp,
    lines: String(d.body || "").split("\n"),
    click: d.click || PAGE_URL,
    clickLabel: "Open auto-pricing",
    foot: "Sent by the Exor Games worker. Who gets this is the AUTOPRICE_EMAIL_TO worker secret; it stops if RESEND_API_KEY is removed.",
  });
  const r = await sendEmail(cx.env, to, note, { replyTo: null, fetchFn: cx.fetch });
  return { ...r, skipped: false, to };
}
...
export async function publishDigest(cx, d, meta) {
  const digest = { at: cx.now(), ...(meta || {}), title: d.title, body: d.body, priority: d.priority, tags: d.tags, click: d.click, sent: false, sentBy: null, sentAt: null, lastError: null, email: null };
  await cx.storage.put("ap:digest", digest);
  // Email first: it is the channel that actually lands. ntfy is still tried
  // every time, so a working topic keeps pushing to the phone as before.
  let e;
  try { e = await sendDigestEmail(cx, d, digest); } catch (err) { e = { ok: false, status: "failed", error: msg(err) }; }
  if (!e.skipped) digest.email = { status: e.status, to: e.to || [], id: e.id || null, error: e.error || null, at: cx.now() };
  let r;
  try { r = await sendNtfy(cx, d); } catch (err) { r = { ok: false, error: msg(err) }; }
  if (r.ok) { digest.sent = true; digest.sentBy = e.ok ? "worker + email" : "worker"; digest.sentAt = cx.now(); }
  else if (e.ok) { digest.sent = true; digest.sentBy = "email"; digest.sentAt = cx.now(); digest.lastError = r.skipped ? null : r.error; }
  else if (r.skipped) { digest.sent = true; digest.sentBy = "nobody (ntfy off)"; digest.sentAt = cx.now(); }
  else digest.lastError = r.error;
  await cx.storage.put("ap:digest", digest);
  return { ok: !!(r.ok || e.ok), ntfyOk: !!r.ok, emailed: !!e.ok, to: e.to || [],
    emailError: e.ok || e.skipped ? null : e.error || "failed",
    skipped: !!(r.skipped && !e.ok), relay: !r.ok && !r.skipped && !e.ok, error: r.error || null };
}
...
async function notifyRun(cx, run, cfg) {
  ...
  try {
    ...
    const by = [r.ntfyOk ? "ntfy" : "", r.emailed ? "email" : ""].filter(Boolean).join(" + ");
    run.notify = by ? "sent (" + by + ")" : r.skipped ? (r.emailError ? "off (email failed)" : "off") : "queued for the relay (" + r.error + ")";
    if (r.emailError) run.errors.push("digest email: " + r.emailError);
  } catch (e) { run.notify = "failed: " + msg(e); run.errors.push("digest: " + msg(e)); }
}
...
export async function serveAutoprice(request, env, url, staffOk) {
  ...
  if (url.pathname === "/autoprice/control" && request.method === "POST") {
    ...
    let flash = "";
    if (body.action === "ntfy-test") { try { const j = JSON.parse(text); const via = [j.ntfyOk ? "ntfy" : "", j.emailed ? "emailed to " + (j.to || []).join(", ") : ""].filter(Boolean).join("; "); flash = j.ok ? "Test digest sent (" + via + "): " + (j.digest && j.digest.title) + (j.emailError ? " · email failed: " + j.emailError : "") : j.relay ? "Direct push refused (" + (j.error || "") + ")" + (j.emailError ? " and the email failed (" + j.emailError + ")" : "") + ". Queued for the relay: it goes out from GitHub on the nightly relay run, or now if you run the ntfy-relay workflow." : "Test digest failed: " + (j.error || j.emailError || "unknown"); } catch { flash = "Test digest: no answer"; } }
