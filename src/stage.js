...
// buylist.js), the payment they chose, and where it stands.
export function shapeRecord(b, now) {
  ...
  return { id, ts, customer, paymentType, cards, totals: totalsOf(cards), repriced, status: "staged", note: "", events: [{ ts, action: "submitted" }], bp: null };
}
...
export async function stageSubmit(env, url, { customer, paymentType, cards, repriced }) {
  const j = await doCall(env, url.origin, "/_stage/put", { customer, paymentType, cards, repriced });
  if (!j.ok) throw new Error(j.error || "could not stage the list");
  try { await notifyStaff(env, url, { id: j.id, customer, paymentType, totals: j.totals }); } catch {}
...
async function notifyStaff(env, url, s) {
  ...
  const body = `${s.totals.units} card${s.totals.units === 1 ? "" : "s"} · ${money(s.totals.cash)} cash / ${money(s.totals.credit)} credit · ${s.paymentType} · customer ${s.customer}`;
...
function renderPage(d, k, env, err) {
  ...
  const cardBlock = (r, editable) => `<div class="card" id="rec-${esc(r.id)}"><h3>${esc(when(r.ts))} <span class="tag tag-${esc(r.status)}">${esc(r.status)}</span> <span class="muted">· ${esc(r.paymentType)} · <a href="${ADMIN_CUSTOMER}${esc(r.customer)}" target="_blank" rel="noopener">customer ${esc(r.customer)}</a>${r.bp && r.bp.number ? " · BinderPOS reference " + esc(r.bp.number) : ""}</span></h3>
...
<p class="muted">Online buylists wait here until a staff member approves them. Approve sends the list to BinderPOS as a new online buylist at today's buylist prices - then it is completed in the BinderPOS portal exactly as before, which is when the customer is paid and the stock rises. Reject keeps it here and sends nothing. Atlantic time. <a href="${STAFF_PAGE}?k=${encodeURIComponent(k)}">refresh</a> · <a href="/portal/buylists?k=${encodeURIComponent(k)}">BinderPOS's list</a></p>
