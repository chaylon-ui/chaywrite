...
<div id="ad-res" class="hits"></div></div>` : ""}
...
function search(){var s=q.value.trim(),st=setIn.value.trim();if(s.length<2&&!st){am.textContent='Type at least 2 letters, or pick a set.';return;}am.textContent='Searching…';res.innerHTML='';
fetch('/buylist/api/search?q='+encodeURIComponent(s)+'&game='+encodeURIComponent(sel.value)+(st?'&set='+encodeURIComponent(st):'')).then(function(r){return r.json()}).then(function(j){var hits=(j&&j.hits)||[];am.textContent=hits.length?'':'Nothing found on the buylist for that.';
res.innerHTML=hits.map(function(h){var byFin={},order=[];(h.variants||[]).forEach(function(v){(v.cardBuylistTypes||[]).forEach(function(p){var f=p.type&&p.type!=='Normal'?String(p.type):'';if(!byFin[f]){byFin[f]=[];order.push(f);}byFin[f].push({v:v,p:p});});});
...
return '<div class="tile">'+(h.imageUrl?'<img src="'+esc(h.imageUrl)+'" alt="" loading="lazy">':'')+'<div class="tn"><b>'+esc(h.cardName)+'</b>'+(f?' <span class="perm">✦ '+esc(f)+'</span>':'')+'<br><span class="muted">'+esc(h.setName)+'</span></div><table class="mini"><thead><tr><th>Condition</th><th class="n">Cash<br><span class="muted">credit</span></th><th class="n">Qty</th></tr></thead><tbody>'+rows+'</tbody></table></div>';}).join('');
}).join('');}).catch(function(e){am.textContent='Search failed: '+e;});}
