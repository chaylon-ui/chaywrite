...
export function renderSheet(r, o) {
  ...
  const line = (c) => `<tr data-key="${esc(key(c))}" data-qty="${esc(c.quantity)}" data-cash="${p2(c.cashBuyPrice)}" data-credit="${p2(c.storeCreditBuyPrice)}">${thumb(c)}<td><b>${esc(c.cardName)}</b><br><span class="muted">${esc(c.setName)}</span></td><td>${esc(c.conditionName || c.condition)}${c.type && c.type !== "Normal" ? "<br><span class=\"fin\">✦ " + esc(c.type) + "</span>" : ""}</td>
