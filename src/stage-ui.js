...
.hz{display:flex;align-items:center;gap:10px;flex-wrap:wrap}.hz h2{margin:0}.sw{font-size:12px;font-weight:800;padding:3px 10px;border-radius:999px;background:#fee2e2;color:#7f1d1d}.sw.on{background:#d1fae5;color:#065f46}.tiles{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin:12px 0}.tile2{background:#fff;border:1px solid #dde3e7;border-radius:12px;padding:10px 14px}.tile2 b{display:block;font-size:22px}.tile2 span{font-size:11px;letter-spacing:.06em;text-transform:uppercase;color:#6b7780}.tile2.warn{border-color:#f0b429;background:#fffbea}.tile2.warn b{color:#92400e}
.find{display:flex;gap:8px;align-items:center;background:#fff;border:2px solid #d52c28;border-radius:12px;padding:10px 14px;margin:0 0 12px;flex-wrap:wrap}.find input.text{flex:1 1 260px;font-size:16px}.find .hint{color:#5f6b76;font-size:13px}.arr .hd{display:flex;align-items:center;gap:10px;padding:10px 14px;background:#f8fafb;border-bottom:1px solid #eef2f4;flex-wrap:wrap}.arr .hd b{font-size:15px}.arr .hd .age{margin-left:auto;font-size:12px;color:#6b7780}.arr .hd .age.old{color:#b45309;font-weight:700}.arr .hd .age.vold{color:#b42318;font-weight:700}.pill{font-size:11px;padding:2px 8px;border-radius:999px;background:#eef2f4;color:#374151;white-space:nowrap}.pill.cart{background:#dbeafe;color:#1e3a8a}.pill.bl{background:#fef3c7;color:#78350f}.heldn{font-weight:800;color:#065f46;background:#d1fae5;border-radius:999px;padding:2px 9px}.rel{display:inline-flex;gap:6px;align-items:center;flex-wrap:wrap}.rel input.q{width:48px}.rel button.dmg{background:#fff;color:#b42318;border:1px solid #b42318}.rel button.go{background:#0d7a5f;color:#fff;border:0}.arr .card{margin-bottom:12px;padding:0;overflow:hidden}.arr table{margin:0}.sw-form{display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin:8px 0}.sw-form input.q{width:60px}.notheld td{color:#6b7780}.hist{list-style:none;margin:0;padding:0}.hist li{padding:5px 0;border-top:1px solid #eef2f4;font-size:13px}.hist li:first-child{border-top:0}.two{display:grid;grid-template-columns:1fr 1fr;gap:12px}@media (max-width:860px){.two{grid-template-columns:1fr}}
...
<td class="n${credit ? " dim" : ""}"${credit && canPrices ? ' title="Locked: this list is paid as Store Credit, so the cash price is not paid"' : ""}>${canPrices && !credit ? `<input class="p cash${c.staffPriced ? " staff" : ""}" type="number" min="0" step="0.01" value="${p2(c.cashBuyPrice)}" data-orig="${p2(c.cashBuyPrice)}">` : money(c.cashBuyPrice) + (c.staffPriced ? ' <span class="muted" title="staff price">*</span>' : "")}</td>
<td class="n${credit ? "" : " dim"}"${!credit && canPrices ? ' title="Locked: this list is paid as Cash, so the credit price is not paid"' : ""}>${canPrices && credit ? `<input class="p credit${c.staffPriced ? " staff" : ""}" type="number" min="0" step="0.01" value="${p2(c.storeCreditBuyPrice)}" data-orig="${p2(c.storeCreditBuyPrice)}">` : money(c.storeCreditBuyPrice)}</td>
...
/* ---------------- Held stock: hold on arrival, stage 2 ---------------- */

export function renderHeld(d, o) {
  const u = o.user, admin = u && u.role === "admin", canRelease = can(u, "release");
  const st = (d && d.settings) || {}, c = (d && d.counts) || {};
  const now = d && d.generatedAt || Date.now();
  const ago = (ms) => { const m = Math.max(0, Math.round((now - ms) / 60000)); return m < 1 ? "just now" : m < 60 ? m + " min ago" : m < 1440 ? Math.round(m / 60) + " h ago" : Math.round(m / 1440) + " day" + (Math.round(m / 1440) === 1 ? "" : "s") + " ago"; };
  const ageClass = (ms) => now - ms > 3 * 86400e3 ? "age vold" : now - ms > 86400e3 ? "age old" : "age";
  const hidden = (k, v) => `<input type="hidden" name="${esc(k)}" value="${esc(v)}">`;
  const ctl = `${BASE}/held/control`;
  const left = (l) => Math.max(0, (l.held || 0) - (l.released || 0) - (l.damaged || 0));
  const status = (l) => l.status === "waiting" ? `<span class="muted">waiting for ${l.remaining} more</span>` : l.status === "no-product" || l.status === "error" ? `<span class="muted" title="${esc(l.note)}">not held</span>` : l.status === "short" || l.status === "never-arrived" ? `<span class="muted" title="${esc(l.note)}">${l.held || 0} of ${l.qty} arrived</span>` : "";
  const line = (r, l, i) => `<tr><td class="t">${l.image && safeImage(l.image) ? `<img class="thumb" src="${esc(safeImage(l.image))}" alt="" loading="lazy">` : ""}</td><td><b>${esc(l.title)}</b>${l.sku ? `<br><span class="muted">${esc(l.sku)}</span>` : ""}</td><td>${esc(l.variantTitle || l.condition)}${/foil/i.test(l.variantTitle || l.finish || "") && !/non/i.test(l.variantTitle || l.finish || "") ? ` <span class="fin">✦ Foil</span>` : ""}</td><td class="n">${left(l) ? `<span class="heldn">${left(l)}</span>` : `<span class="muted">0</span>`} <span class="muted">of ${l.qty}</span> ${status(l)}</td><td class="n">${canRelease && left(l) ? `<form method="post" action="${ctl}" class="rel">${hidden("key", r.key)}${hidden("line", i)}${hidden("q", o.q || "")}<input class="q" type="number" name="n" min="1" max="${left(l)}" value="${left(l)}"><button class="sm go" name="action" value="release">Release</button><button class="sm dmg" name="action" value="damaged" onclick="return confirm('Mark these copies damaged? They move to Shopify\'s damaged count and never reach the shelf.')">Damaged</button></form>` : ""}</td></tr>`;
  const arrival = (r) => `<div class="card arr"><div class="hd"><span class="pill ${r.kind === "cart" ? "cart" : "bl"}">${r.kind === "cart" ? "POS cart " : "Buylist "}${esc(r.ref)}</span><b>${esc(r.customer || "Walk-in")}</b>${r.till ? `<span class="muted">${esc(r.till)}</span>` : ""}${r.who ? `<span class="muted">rang up by ${esc(r.who)}</span>` : ""}${r.paid && r.paid.total ? `<span class="muted">paid ${money(r.paid.total)}${r.paid.credit && !r.paid.cash ? " store credit" : r.paid.cash && !r.paid.credit ? " cash" : ""}</span>` : ""}${r.link ? `<a href="${esc(r.link)}" target="_blank" rel="noopener">BinderPOS ↗</a>` : ""}<span class="${ageClass(r.ts)}">arrived ${esc(ago(r.ts))} · ${esc(when(r.ts))}</span>${canRelease && r.held ? `<form method="post" action="${ctl}">${hidden("key", r.key)}${hidden("q", o.q || "")}<button class="sm" name="action" value="release-all" onclick="return confirm('Release all ${r.held} held cards from this arrival to the shelf?')">Release all ${r.held}</button></form>` : ""}</div>
<table><thead><tr><th class="t"></th><th>Card</th><th>Condition</th><th class="n">Held</th><th class="n">${canRelease ? "Release" : ""}</th></tr></thead><tbody>${(r.lines || []).map((l, i) => line(r, l, i)).join("")}</tbody></table></div>`;
  const matches = d && d.matches;
  const switchBox = admin ? `<div class="card"><h3>The switch</h3><p class="muted" style="margin:0 0 6px">While on, every buy cart rung up in BinderPOS and every buylist completed there is held: the cards go into Shopify's quality-control count instead of the shelf, and come back here to be released. Turning it off releases nothing.</p>
${st.on ? `<form method="post" action="${ctl}" class="sw-form"><button class="sm" name="action" value="off" onclick="return confirm('Switch the hold off? What is held stays held until released.')">Switch OFF</button>${st.trial ? `<span class="muted">Trial: ${st.trial} more arrival${st.trial === 1 ? "" : "s"}, then it switches itself off.</span>` : `<span class="muted">On since ${esc(when(st.since))} by ${esc(st.by)}.</span>`}<button class="sm" name="action" value="check" title="look at BinderPOS now instead of in two minutes">Check BinderPOS now</button></form>` : `<form method="post" action="${ctl}" class="sw-form"><label>Try it on the next <input class="q" type="number" name="trial" min="0" max="500" value="2"> arrival(s)</label><button class="sm" name="action" value="on">Switch ON</button><span class="muted">0 = stay on until switched off. A trial switches itself off after that many carts or buylists.</span></form>`}
${st.lastPoll ? `<p class="muted" style="margin:6px 0 0">Last look at BinderPOS ${esc(when(st.lastPoll))}${st.lastError ? ` · <span style="color:#b42318">${esc(st.lastError)}</span>` : ""}.</p>` : ""}</div>` : "";
  const body = `<div class="hz"><h2>Held stock</h2><span class="sw${st.on ? " on" : ""}">Hold on arrival: ${st.on ? "ON" + (st.trial ? " · trial, " + st.trial + " left" : "") : "OFF"}</span></div>
<p class="muted">Cards bought at the counter or through a buylist wait here, off the website and hidden from the POS, until the card room releases them.</p>
${o.err ? `<div class="err">${esc(o.err)}</div>` : ""}${o.msg ? `<div class="okmsg">${esc(o.msg)}</div>` : ""}
<div class="tiles"><div class="tile2"><b>${c.held || 0}</b><span>cards held</span></div><div class="tile2"><b>${c.arrivals || 0}</b><span>arrivals waiting</span></div><div class="tile2${c.old24 ? " warn" : ""}"><b>${c.old24 || 0}</b><span>older than 24 h</span></div><div class="tile2"><b>${c.releasedToday || 0}</b><span>released today</span></div><div class="tile2"><b>${c.notHeld || 0}</b><span>could not be held</span></div></div>
${canRelease ? `<form method="get" action="${BASE}/held" class="find"><input class="text" type="search" name="q" value="${esc(o.q || "")}" placeholder="Customer at the counter? Type a card name, SKU or barcode to find it among the held cards"><button class="sm" type="submit">Find</button><span class="hint">then Release 1</span></form>` : ""}
${matches ? `<div class="card"><h3>Found ${matches.length} held line${matches.length === 1 ? "" : "s"} for “${esc(o.q)}”</h3>${matches.length ? `<table><tbody>${matches.map((m) => `<tr><td class="t">${m.image && safeImage(m.image) ? `<img class="thumb" src="${esc(safeImage(m.image))}" alt="">` : ""}</td><td><b>${esc(m.title)}</b><br><span class="muted">${esc(m.variantTitle)} · ${m.kind === "cart" ? "POS cart " : "buylist "}${esc(m.ref)} · ${esc(ago(m.ts))}</span></td><td class="n"><span class="heldn">${m.held}</span></td><td class="n"><form method="post" action="${ctl}" class="rel">${hidden("key", m.key)}${hidden("line", m.line)}<input class="q" type="number" name="n" min="1" max="${m.held}" value="1"><button class="sm go" name="action" value="release">Release</button></form></td></tr>`).join("")}</tbody></table>` : `<p class="muted">Nothing held matches. It may already be on the shelf, or it was never held (see "Could not be held").</p>`}</div>` : ""}
${switchBox}
${(d.open || []).length ? d.open.map(arrival).join("") : `<div class="card"><p class="muted" style="margin:0">Nothing is held right now.${st.on ? " The next buy cart or completed buylist will appear here within two minutes." : ""}</p></div>`}
<div class="two"><div class="card"><h3>Could not be held <span class="muted">· last 7 days</span></h3>${(d.notHeld || []).length ? `<table class="notheld"><tbody>${d.notHeld.map((x) => `<tr><td>${esc(x.title)} ${x.condition ? "· " + esc(x.condition) : ""} ×${x.qty}</td><td>${x.kind === "cart" ? "POS cart " : "buylist "}${esc(x.ref)}</td><td class="n">${esc(x.note || x.status)}</td></tr>`).join("")}</tbody></table>` : `<p class="muted" style="margin:0">Nothing. Every line the hold saw had a Shopify product and arrived.</p>`}</div>
<div class="card"><h3>History <span class="muted">· last 7 days</span></h3>${(d.history || []).length ? `<ul class="hist">${d.history.map((h) => `<li>${esc(when(h.ts))} · ${esc(h.by || "hold")} · ${esc(h.text)}</li>`).join("")}</ul>` : `<p class="muted" style="margin:0">Nothing yet.</p>`}</div></div>
${(d.done || []).length ? `<div class="card"><h3>Released in the last 24 hours</h3>${d.done.map((r) => `<p style="margin:4px 0">${r.kind === "cart" ? "POS cart " : "Buylist "}${esc(r.ref)} · ${esc(r.customer || "Walk-in")} · ${(r.lines || []).reduce((a, l) => a + (l.released || 0), 0)} released${(r.lines || []).some((l) => l.damaged) ? ", " + (r.lines || []).reduce((a, l) => a + (l.damaged || 0), 0) + " damaged" : ""} · done ${esc(when(r.doneAt))}</p>`).join("")}</div>` : ""}
<p class="note"><b>Off switch:</b> pausing the hold never releases anything by itself, and Release still works while it is off.</p>`;
